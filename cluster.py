...
# %%
for k in tqdm(range(num_city, num_pct, 20)):
    c = AgglomerativeClustering(n_clusters=k)
    y = c.fit_predict(df_CPW[["lat", "lon"]])
    scores[k] = silhouette_score(df_CPW[["lat", "lon"]], y)
    labels[k] = y
...
df_CPW["label"] = labels[best_k]
for r in df_CPW.to_dict("records"):
    folium.CircleMarker(
...
# %%
df_CPW["label"].value_counts()


# %% pick a label and visualize the datapoints on map
biggest_cluster = df_CPW["label"].value_counts().index[0]
m = folium.Map((40.7128, -74.0060))
for r in df_CPW[df_CPW["label"] == biggest_cluster].to_dict("records"):
    folium.CircleMarker(
        location=(r["lat"], r["lon"]), radius=1, color=colors[r["label"]]
    ).add_to(m)
